...
\label{sec:org324a9a8}
\subsection{The invisible が carriage}
\label{sec:org5b647fa}
...
\label{sec:org5dc1366}
\subsection{は particle}
\label{sec:orgb6f3b64}
...
\label{sec:org5c96d3d}
\subsection{Negative nouns}
\label{sec:org72eb654}
...
\label{sec:org8311af4}
\subsection{Try - と する}
\label{sec:orgadc16a7}
\begin{quote}
...
\label{sec:org56bc3e5}
\subsection{Likeness}
\label{sec:org16ce68f}
...
\label{sec:org09b6638}
\subsection{だけ}
\label{sec:org99d9842}
...
\label{sec:orga9e34b9}
\subsection{する verbs}
\label{sec:org80dce43}
\begin{quote}
...
\label{sec:org6681d13}
\subsection{ね}
\label{sec:org6a9d134}
...
\label{sec:orgd337383}

\section{Lesson 46: Japanese word order}
\label{sec:org69b6661}
...
\section{Lesson 51: How to read a Japanese Kaidan (ghost story)}
\label{sec:org90e386f}
\#+BEGIN\textsubscript{QUOTE}:
それは ある女の子が先輩のアパートで行われた飲み会に参加した日
\#+END\textsubscript{QUOTE}
\begin{itemize}
\item それは is the equivalent of English 'it was'. There is no 'it' in Japanese, so when we want to introduce a scene (like in the start of a story) and say 'it was a cold night' or something similar we use 'that' -- 'as for that\ldots{}' = 'it was\ldots{}'
\item 日 It is not immediately clear what the engine of this sentence is, there is no だ, no adjective and no verb. However, since it ends in a noun we can know that this is an A is B sentence, and that the copula (だ) has been dropped. This happens very often in casual speech.
\item The sentence is tenseless (no copula to tell), we can tell the tense however because the purpose of this sentence is to set the scene for a story, so it'd be in the past tense.
\item Sentence core: 'It was a day'
\end{itemize}
...
\begin{itemize}
\item This entire block is a single modifier for the day. We know this because a verb (the engine of the clause) connects directly to a noun.
\item The core of this clause is clearly 「女の子が参加した」 - Girl who took part
\item Remember ある here means 'a certain'
\item 飲み会 - drinking party
\item 先輩のアパートで - at (the action 行う was made possible by) Senpai's apartment
\item 行われた - perform/do/conduct (held)
\end{itemize}
...
\section{Lesson 52: In-depth japanese sentence analysis}
\label{sec:orgc90de6c}
\begin{quote}
読み会が終了したあと
\end{quote}
\begin{itemize}
\item 読み会 - Drink party
\item 終了 - end; close; termination
\end{itemize}
...
\end{quote}
\begin{itemize}
\item 引き返し - Particularly in narrative, we can use the い-stem of a verb much the same was as the て-form of a verb, to complete a clause and connect to the next
\item Note again how we are using present-tense sentences in a past-tense story. This is normal for Japanese stories to add immediacy.
\end{itemize}
...
\end{quote}
\begin{itemize}
\item ところが is a common expression in narrative. The が is not the particle but the contrastive conjunction 'but'. ところ which means place can also mean a place in time, so the combined meaning is 'at that time, then\ldots{}', 'But then'.
\item そのまま - (In) that unchanged state\ldots{} 'Because the door was unlocked\ldots{}' in that (her) unchanged state (just like that) '\ldots{} she went in'
\begin{itemize}
\item 'Because the door was unlocked she went in \textbf{just like that}'
\end{itemize}
\end{itemize}

\section{Lesson 53: Enjoy Japanese Horror (more sentence analysis)}
\label{sec:orgef7e5a6}
\begin{quote}
部屋の中は電気がついておらず
\end{quote}
\begin{itemize}
\item 電気をつける - Turn on the lights
\begin{itemize}
\item おらず - おる = いる - ず = ない
\begin{itemize}
\item Both this おる and ず are old fashioned (or けいご (polite) (and sometimes dialectical)) japanese
\begin{itemize}
\item 電気がついておらず - The electricity existed in a state of not having been switched on - The lights were off
\end{itemize}
\end{itemize}
\end{itemize}
\end{itemize}
...
\end{quote}
\begin{itemize}
\item どうやら - It seems/It appears, working in tandem with the らしい at the end of the sentence, somehow \ldots{} it appeared
\end{itemize}
...
\end{quote}
\begin{itemize}
\item 無用心 - Acting without caution esp. w.r.t. crime or danger (examples given include walking down a dark street at night or leaving valuables on display in one's car)
\begin{itemize}
\item だな - As we know な is the particle used for directing a comment at oneself
\end{itemize}
\item 起こそうかと思った - 起こす (wake, other-move) - 起こそう (volitional form (will to do)) - She thought about possibly waking [senpai]
\item が - but (end of a clause, can't be the other が)
\end{itemize}
...
\end{quote}
\begin{itemize}
\item かなり is an adverbial noun (one that doesn't need に or と to modify a verb/adjective) meaning sufficiently/pretty/very
\item の bundles everything that came before it into a noun
\begin{itemize}
\item 思い出し - Remembered, Note: い-stem as a clause-connector
\end{itemize}
\item そとする means to act quietly, そと (quiet) + する (act (do))
\begin{itemize}
\item おく means to put something in a place, but as we know the japanese concept of 'place' is very loose, so そとしておく means 'put in place acting quietly' (fairly close to a common English expression)
\end{itemize}
\end{itemize}
\end{document}
